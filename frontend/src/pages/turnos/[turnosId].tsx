...
const EditTurnos = () => {
  ...
  const { turnosId } = router.query;

  useEffect(() => {
    dispatch(fetch({ id: turnosId }));
  }, [turnosId]);
  ...
  useEffect(() => {
    if (typeof turnos === 'object') {
      ...
      Object.keys(initVals).forEach((el) => (newInitialVal[el] = turnos[el]));
  ...
  const handleSubmit = async (data) => {
    await dispatch(update({ id: turnosId, data }));
...
EditTurnos.getLayout = function getLayout(page: ReactElement) {
  return (
