...
module.exports = class Localizacion_taxisDBApi {
  ...
  static async update(id, data, options) {
    ...
    if (data.ultima_actualizacion !== undefined)
      updatePayload.ultima_actualizacion = data.ultima_actualizacion;

    updatePayload.updatedById = currentUser.id;

    await localizacion_taxis.update(updatePayload, { transaction });

    if (data.taxi !== undefined) {
      await localizacion_taxis.setTaxi(
        data.taxi,
  ...
  static async findAll(filter, globalAccess, options) {
    ...
    try {
      const { rows, count } = await db.localizacion_taxis.findAndCountAll(
        queryOptions,
      );
